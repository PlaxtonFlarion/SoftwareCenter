...
- `xtra`：外接 MCP 工具、Mind native 工具与编码工具协作通道，走独立 `mode=xtra` 后端链路；Helix MCP 由 `--mcp` 或 `/helix-link` 显式接入后追加
...
- `docs/playbook.load.md`：本地脚本执行、压测链路与托管边界

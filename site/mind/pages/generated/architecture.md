...
主 README 只负责项目定位和最小摘要；完整背景说明继续看这里。
重点是解释 Mind 为什么要做成现在这个形态，以及控制面、执行面、工具域和云端增强分别承担什么职责。

## 先判断是不是这页的范围

- 你要理解项目为什么不是传统自动化脚本或 RPA：看这里
- 你要理解 `Mind / Helix / device / bench / common / media` 这些层和域怎么分：看这里
- 你要理解端侧执行和云端增强为什么分离：看这里
- 你只是想先跑起来或确认模式边界，先回 README，不必一上来就读完整架构文档

## 怎么读这页

- 先看“设计原则”和“架构总览”，建立项目骨架
- 再看“工具域与能力边界”和“执行闭环与证据链”，理解系统怎么落地执行
...
补充：
- Mind 除了 `chat / fast / plan` 三种本地主动执行通道，还提供 `agent` 驻留订阅入口
- `agent` 负责向服务端注册驻留会话，并把远端下发任务映射回本地 `chat / fast / plan` 执行面
- 具体协议时序、恢复链路和排障不在这页展开，另见 `驻留与订阅模式`

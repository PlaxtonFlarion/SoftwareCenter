...
入口页只负责项目定位和最小摘要；完整背景说明继续看这里。
重点是解释 Mind 为什么要做成现在这个形态，以及控制面、执行面、主要能力方向和云端增强分别承担什么职责。

## 先判断是不是这页的范围

- 你要理解项目为什么不是传统自动化脚本或 RPA：看这里
- 你要理解 `Mind / Helix` 和主要能力怎么分：看这里
- 你要理解端侧执行和云端增强为什么分离：看这里
...
- **约束优先**：模型轮次使用流式 `tool_call -> tool_result` 闭环，并由工具元数据、沙箱和审批策略约束执行
...
### 分层与职责
- **控制面（Mind）**：指令解析、会话编排、模型调度、任务路由与结果聚合
- **执行面（Helix）**：工具注册与发现、调用生命周期管理、域隔离、运行态监测、keepalive 和证据链落盘

补充：
- Mind 提供统一主动执行链路和独立的 `agent` 订阅入口
- `agent` 负责向服务端注册会话，并把远端下发任务映射回本地统一执行入口
...
└─────┬──────┘
      │
      ▼
Tools: automator / bench / common / coding / media
```

### 执行通道
- **主动执行**：`exec` 和交互会话共用统一模型轮次与工具生命周期
- **远程订阅**：`agent listen` 接收服务端任务并复用统一执行入口
- **工具接入**：native tools、外接 MCP 与按需连接的 Helix 共同组成当前工具目录
...
示例接口：
- Helix JSON API：`/ready`、`/healthz`、`/version`、`/api/idle`、`/api/keepalive`、`/api/runtime/exec-env`、`/api/logs`、`/api/pref`、`/api/service-config`、`/api/agent`
...
- 视觉模型负责关键帧状态确认与目标定位，并为设备与 UI 执行提供可视锚点

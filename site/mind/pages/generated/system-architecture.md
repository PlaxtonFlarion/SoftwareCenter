...
Mind 在模型 Profile 中声明可选的上下文窗口和自动压缩 token 阈值，并通过模型请求契约提交。
AppServer 根据所选模型配置解析有效值、校验输出预留并写入 Turn 配置快照；执行与接管沿用
快照中的窗口和阈值。自动压缩以当前模型输入的 token 预算触发，不以固定用户轮数或 Item 数
触发。模型容量未知时必须由客户端或该模型的服务端配置显式声明，不套用其它模型的容量。

...
- 设备 / UI 执行：从 [快速开始](#quick-start)、[运行模式](#modes) 和 [设备域实战](playbook.device.md) 入手
...
- 驻留任务下发 / 长链路监听：看 [驻留与订阅模式](agent-mode.md)
...
- 已经知道要找什么：直接走 [Docs 索引](docs-index.md)
- 准备看协议、模板和安全链路：从 [接口实战](#playbook-api)、[模板能力](playbook.template.md)、[安全工具](playbook.security.md) 继续
- 准备看设备、多媒体和稳定性链路：从 [设备域实战](playbook.device.md)、[Monkey 扰动](playbook.monkey.md)、[多媒体链路](#playbook-media) 继续
...
- `agent` 不属于 REPL 内的第四个状态；协议细节见 [驻留与订阅模式](agent-mode.md)
...
- 需要驻留监听：再看 `--agent` 和 [docs/agent-mode.md](agent-mode.md)
- 想留痕归档：再看 `--gravity`
- 想调试链路：再看 `--reflection`
- 想批跑和回归：最后看 `--code`
...
- 协议时序、消息类型和排障细节，直接看 [docs/agent-mode.md](agent-mode.md)
...
- 真正的字段、层级和规则写法，直接看 [docs/cli-code.md](cli-code.md)
...
高级批跑说明已拆到独立文档：[docs/cli-code-advanced.md](cli-code-advanced.md)。
...
## ⭐️ 性能实战
完整性能案例和星图样例已拆到独立文档：[docs/playbook.performance.md](playbook.performance.md)。
...
<a id="playbook-api"></a>
## ⭐️ 接口实战
完整协议约定、helper 迁移和样例说明已拆到独立文档：[docs/playbook.api.md](playbook.api.md)。
...
需要字段级骨架、`env / items / concurrency / fail_fast` 的真实写法时，直接看 [docs/cli-code.md](cli-code.md) 和 [docs/playbook.api.md](playbook.api.md)。
...
## ⭐️ 多媒体链路
完整媒体命令、组合链路和星图任务已拆到独立文档：[docs/playbook.media.md](playbook.media.md)。

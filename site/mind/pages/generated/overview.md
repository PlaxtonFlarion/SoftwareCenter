...
- 开发或维护项目：先看[系统架构](system-architecture.md)，再看[客户端架构](client-architecture.md)和[正文目录](docs-index.md)

**专题跳转**

- CLI 与交互：[命令行参考](cli-usage.md) · [交互模式](interactive-mode.md)
- MCP 与订阅：[MCP Server](mcp-server.md) · [订阅模式](agent-mode.md)
- 工程能力：[原生 coding](playbook.nativecoding.md) · [设备与 UI](playbook.device.md) · [多媒体](playbook.media.md)
- 稳定性与维护：[性能实战](playbook.performance.md) · [Hooks](hooks.md) · [维护者指南](maintainer-guide.md)
...
完整参数、stdin、图片、模型覆盖、补全和多级帮助见[命令行使用参考](cli-usage.md)。
...
[CLI OAuth 登录](cli-usage.md#oauth-浏览器登录)。运行时可恢复凭据并自动刷新，
工具调用继续使用既有审批策略。支持范围和恢复方式见该说明；运行入口与真实服务的验证步骤见
[OAuth 验收指南](mcp-oauth-acceptance.md)。
...
不要在 Mind 自己的 `[mcp_servers]` 中注册 `mind mcp-server`，否则会递归启动自身。完整说明见 [Mind MCP Server](mcp-server.md)。

<a id="interactive-mode"></a>
## ⭐️ 交互模式

启动 `mind` 进入交互模式。常用入口包括 `/new`、`/resume`、`/review`、`/permissions`、`/preferences`、`/tools`、`/mcp`、`/helix-link` 和 `/quit`；完整 slash 命令及运行中行为见[交互模式文档](interactive-mode.md)。
...
- [系统架构](system-architecture.md)：Mind、AppServer 与 Fabric 的职责、Authority 和跨仓生命周期
- [客户端架构](client-architecture.md)：Mind 内部包边界、依赖方向、状态所有权和生命周期
- [架构评分卡](architecture-scorecard.md)：阶段性架构评审和成熟度记录，不定义架构事实
- [产品背景与能力生态](architecture.md)：产品定位、使用场景和可选能力，不定义架构事实
...
提交代码前请先阅读 [AGENTS.md](agents.md)、[系统架构](system-architecture.md)、
[客户端架构](client-architecture.md)和[维护者指南](maintainer-guide.md)，并根据影响范围运行定向测试和文档校验。

...
- 如果你要把设备动作真正写进 `mind flow`，结构层先配合星图协议页，执行语义再配合星图深入说明
...
设备与 UI 执行负责真的操作设备；多媒体处理负责截图、录屏、音视频和证据加工；协议与校验、性能采样负责接口与报告链路；`mind flow` 负责把动作组织成可批跑、可回归的星图。

...
重点是先判断当前任务是不是落在 `bench.nexus`，再给出统一字段边界和协议入口。
...
## 怎么读这页
- 先看协议覆盖，确认当前场景是否真的落在 `bench.nexus`
- 再看统一约定和字段边界，用同一套结构组织单请求或批量请求
- 最后按协议挑样例，不要把所有协议从头读到尾
- 如果你在写 `--code`，再回头配合 `cli-code.md` 一起读
...
如果你要把接口工具真正写进 `--code`，推荐先看 [星图协议](cli-code.md) 里的“复杂接口星图怎么读”。
那里重点讲的是星图如何编排步骤，不是再重复协议字段表。

...
这一页讲的是：在 `exec --mode xtra` 模式下，怎么把浏览器当成一个可交互、可取证、可排障的外部执行面来用。
...
```bash
mind exec --mode xtra "打开 example.com，搜索 pricing，然后截图"
```
...
```bash
mind exec --mode xtra "打开 https://example.com，抓取当前页面结构，并总结主标题、主按钮和顶部导航"
```
...
```bash
mind exec --mode xtra "打开目标站点，在搜索框里输入 payroll，进入结果页后等待稳定，再总结页面主信息"
```
...
```bash
mind exec --mode xtra "打开登录页，填写账号和密码，提交后等待首页稳定，再截图留证"
```
...
```bash
mind exec --mode xtra "打开工单创建页，填写表单并提交；如果失败，读取控制台和网络请求，定位原因"
```
...
```bash
mind exec --mode xtra "打开生产环境首页，再新开标签页打开 staging，对比两边 header、导航和首屏按钮差异"
```

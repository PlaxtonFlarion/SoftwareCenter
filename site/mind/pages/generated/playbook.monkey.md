...
- 需要继续观察时，间隔调用 `monkey_status`
- 需要主动收束时，调用 `monkey_stop`
...
状态查询、停止和清理属于会话管理动作，本身不需要你再重复描述这些启动意图。
...
如果按会话方式使用，启动动作会先返回；后续再通过状态查询或停止去管理这次运行。
...
- 跑前先确认包名正确
- 需要复现问题时固定 `seed`
- 优先按“启动 -> 状态 -> 停止”的会话方式使用，而不是把它当普通短动作

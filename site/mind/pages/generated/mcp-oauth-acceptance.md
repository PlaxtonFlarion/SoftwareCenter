...
| 手动回调 | 从交互终端执行 `login --manual`，在另一台机器的浏览器授权后，将完整回调 URL 粘贴回隐藏输入；确认无回显、正常保存。另验证浏览器回调先到时能自动结束输入。 |
...
在仓库根目录运行真实 PTY 驱动，使用源码 CLI、本机 HTTP OAuth 故障服务和系统凭据库，
不需要真实账号。验收目录必须尚不存在；结束时清理本轮合成凭据。

```powershell
python -m tests.manual.mcp_callback_acceptance --directory "<NEW_ACCEPTANCE_ROOT>"
```

它检查普通粘贴、括号粘贴、浏览器回调、错误 state/issuer、拒绝、超限、未结束粘贴、
取消、EOF、等待超时、令牌交换取消/超时及跨进程退出竞争。每项都检查原始终端模式恢复、
后续普通输入可用、输入无回显、回调端口关闭和凭据提交结果；报告只记录这些事实。
本机故障服务的粘贴成功不能代替真实 Sentry 或跨机器远程终端验收；实际使用的入口和
平台须分别记录。Linux/macOS 需在对应操作系统运行后才可记为通过。

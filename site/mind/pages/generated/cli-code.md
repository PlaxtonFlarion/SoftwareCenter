...
`mind flow` 用于装载一个或多个批量执行星图，并按显式选定的主模式执行。
重点是回答两件事：什么时候值得写星图，以及一份星图的最小结构应该怎么组织。

先记住一条硬约束：

- `mind flow` 必须提供至少一个执行源
- 必须通过 `--mode chat|fast|xtra` 显式选择运行模式
- 缺少执行源或 `--mode` 会直接报错

当前本地入口除了文件路径，也支持：

- `mind flow - --mode chat`：从标准输入读取星图
- `mind flow inline:... --mode chat`：直接执行内联星图文本
- `mind flow https://... --mode chat`：从 URL 拉取星图文本

最小正确示例：

```text
mind flow --mode chat cases.md
mind flow --mode fast media.md
...
- 只是临时跑一条任务：先用入口页里的基础写法，不必先写 `mind flow`
...
边界说明：
- 这里的 `global_rule / rule` 属于 `mind flow` 的星图规则层
...
`mind flow` 里的星图，不是“再造一套协议参数”，而是把多步工具调用写成一份可批跑、可回放、可加规则的执行脚本。

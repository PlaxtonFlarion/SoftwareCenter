# 参考文档

这页只负责导流：先帮你判断该去哪，再进入正文目录或专题摘要。

<div class="mind-panel-grid mind-panel-grid-tight">
  <div class="mind-panel mind-panel-accent">
    <span class="mind-kicker">先这样读</span>
    <h3>先定目标，再进正文</h3>
    <p>这页只负责把你送进对应正文，不重复讲快速开始、能力判断或星图结构。</p>
  </div>
  <div class="mind-panel">
    <span class="mind-kicker">这页不做什么</span>
    <ul class="mind-mini-list">
      <li>不重复快速开始</li>
      <li>不重复能力概览</li>
      <li>不替代专题正文</li>
    </ul>
  </div>
</div>

## 先走哪条阅读路径

<div class="mind-route-grid">
  <a class="mind-route-card" href="../getting-started/">
    <span class="mind-route-meta">Path A</span>
    <h3>我还没跑起来</h3>
    <p>先看环境、管理中心和最小命令，不要直接跳长文。</p>
    <span class="mind-card-arrow">去快速开始 →</span>
  </a>
  <a class="mind-route-card" href="../generated/overview/">
    <span class="mind-route-meta">Path B</span>
    <h3>我先想看整体入口</h3>
    <p>先扫项目总览，再决定要不要继续看能力概览。</p>
    <span class="mind-card-arrow">去项目总览 →</span>
  </a>
  <a class="mind-route-card" href="../generated/docs-index/">
    <span class="mind-route-meta">Path C</span>
    <h3>我已经知道要找什么</h3>
    <p>直接走正文目录，按主题跳文档最快。</p>
    <span class="mind-card-arrow">去正文目录 →</span>
  </a>
  <a class="mind-route-card" href="../generated/catalog/">
    <span class="mind-route-meta">Path D</span>
    <h3>我想按专题浏览</h3>
    <p>先看专题摘要，再决定要不要进对应正文。</p>
    <span class="mind-card-arrow">去专题摘要 →</span>
  </a>
</div>

## 按执行面继续读

<div class="mind-card-grid">
  <div class="mind-card">
    <h3>chat</h3>
    <p>适合探索、问答、临时诊断和混合型任务。</p>
    <ul class="mind-mini-list">
      <li>先看项目总览，确认整体边界</li>
      <li>再按主题跳设备、协议或多媒体正文</li>
    </ul>
  </div>
  <div class="mind-card">
    <h3>fast</h3>
    <p>适合接口、媒体、文件类短链路任务。</p>
    <ul class="mind-mini-list">
      <li>优先看接口实战和多媒体链路</li>
      <li>需要留证或归档时，再补看命令参数</li>
    </ul>
  </div>
  <div class="mind-card">
    <h3>plan</h3>
    <p>适合巡检、固定流程、批跑和回归。</p>
    <ul class="mind-mini-list">
      <li>优先看星图协议和星图深入说明</li>
      <li>需要样例时，再看星图样例和专题正文</li>
    </ul>
  </div>
</div>

## 按任务类型找正文

<div class="mind-card-grid mind-card-grid-compact">
  <a class="mind-card mind-card-link" href="../generated/playbook.device/">
    <h3>设备与 UI</h3>
    <p>应用生命周期、页面交互、滚动点击和执行留证。</p>
    <span class="mind-card-arrow">查看正文 →</span>
  </a>
  <a class="mind-card mind-card-link" href="../generated/playbook.api/">
    <h3>接口与协议</h3>
    <p>HTTP、SSE、WebSocket、GraphQL 和批量请求结构。</p>
    <span class="mind-card-arrow">查看正文 →</span>
  </a>
  <a class="mind-card mind-card-link" href="../generated/playbook.media/">
    <h3>多媒体</h3>
    <p>抽帧、裁剪、转码、音轨处理和证据链任务。</p>
    <span class="mind-card-arrow">查看正文 →</span>
  </a>
  <a class="mind-card mind-card-link" href="../generated/playbook.performance/">
    <h3>性能与稳定性</h3>
    <p>Framix、Memrix、Monkey 与性能回归链路。</p>
    <span class="mind-card-arrow">查看正文 →</span>
  </a>
</div>

## 按常见目的进入

<div class="mind-panel-grid mind-panel-grid-tight">
  <a class="mind-panel mind-panel-link" href="../getting-started/">
    <span class="mind-kicker">目的一</span>
    <h3>我想先跑通一条命令</h3>
    <p>先去快速开始，把环境、管理中心和最小命令跑通。</p>
  </a>
  <a class="mind-panel mind-panel-link" href="../generated/cli-code/">
    <span class="mind-kicker">目的二</span>
    <h3>我想写 <code>--code</code></h3>
    <p>先读星图协议，再补星图深入说明和样例。</p>
  </a>
  <a class="mind-panel mind-panel-link" href="../generated/agent-mode/">
    <span class="mind-kicker">目的三</span>
    <h3>我想接远端下发任务</h3>
    <p>直接进订阅模式，看会话、长链路和恢复链路。</p>
  </a>
</div>

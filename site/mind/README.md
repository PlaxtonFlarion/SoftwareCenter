...
- `website/mind/docs_manifest.json` 负责登记全部 `docs/` 正文的发布目标，以及官网专题目录摘要
...
- 如果新增或下线正文文档，同步更新 `website/mind/docs_manifest.json`；默认检查和同步入口都会拒绝遗漏的正文、缺失的源文件及未登记的本地文档链接
- 文档 PR 会运行回归测试和完整的生成前后校验；合入 `main` 后，只有校验通过才会同步到公共仓库

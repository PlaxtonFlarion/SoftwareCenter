...
删除生命周期由 `RootConversationSession.delete_session()` 持有：在与提交、绑定、重置和归档
共用的锁内核验冻结身份，收束子代理并持久化根身份、完整集合和请求 ID，再提交正式远端删除。
确认完整回执后退休运行身份、完成资源关闭及 `deleted` 结束 Hook，再调用本地清理；关闭失败
保留计划。未知结果阻止复用目标，恢复只查询原请求并复用关闭流程，不生成新删除身份。
不同请求的重叠目标由 Store 原子拒绝。前端只消费 `SessionDeletionResult`，不得直接调用协议
客户端或本地 Store；恢复其他会话不能切换或结束当前会话。
...
菜单语言由当前 TUI runtime 持有，配置事实沿 `ConfigSession` 保存并在启动时注入。
补全器显式读取语言，展示文案从双语目录派生；会话新建、分支和恢复不持有或覆盖语言状态。
命令名、参数及执行策略独立于展示语言，语言切换复用本地动作和菜单生命周期。

功能层构造 `MenuRequest` 时从双语目录解析标题、选项、标签页和其他业务展示字段。
公共确认、关闭页脚使用 `MenuFooterPreset` 声明用途；搜索字段的 `None` 表示公共默认文案，
显式空字符串表示隐藏。菜单准备阶段显式接收当前语言，派生公共文案及选项 `status_suffix`，
再解析 keymap 和清理终端文本；纯渲染只消费展示值，不读取配置或翻译业务字符串。
新打开或有效更新的菜单读取当前语言；已有模态视图保留展示快照，语言切换不重放回调。
异步功能在提交更新时重新读取语言并构造业务展示值，更新仍受 session、view_id 和 generation
检查约束。默认搜索包含显示标签，并沿用详情或显式搜索键；自定义 matcher、ranker 保留自己的规则。
终端应用 adapter 从已绘制尺寸开始轮询，避免启动后首次快速缩放漏掉重绘；轮询任务仍由
prompt-toolkit 应用生命周期创建和取消，布局尺寸与菜单焦点继续由现有 Screen、Menu 所有者管理。
...
Diff 分页和收件箱详情保存本次展示语言快照；收件箱更新按稳定消息标识保留选中项，
重建展示值。操作进度和完成回调读取 runtime 当前语言，前台任务的去重键独立于展示标签。
MCP 启动快照携带现有的单服务／全部服务目标，不在基础设施层预先拼接操作范围摘要。
共享终端认证、MCP 状态与下载渲染器只消费显式文案值；默认英文供 CLI 使用，TUI 负责构造双语标签，
共享渲染器不读取 TUI 配置。

本地压缩提示由 `CompactNotice` 携带种类与原始详情，Review 状态视图通过 `ReviewNotice` 携带
原始参数，分支请求异常通过本地 `notice_kind` 经协议 adapter 传递。前端按这些类型和正式错误码
生成提示，不按英文正文识别错误；原始远端错误、Hook 原因及模型审查正文保留原文。
这些提示只属于本地结果或展示生命周期，不扩展线上字段，也不能改变远端终态、重试或继续执行决策。
...
抑制，Review 的 Turn 占用和子代理不写入主会话用量；inline Review 的空 Turn ID 累计事件
更新共享 Session 快照。冷恢复及历史裁剪通过注入的用量恢复端口读取
既有报告授权和分页回放中的保留快照；该读取有超时边界，不推进聊天确认游标。TUI 在
恢复目标尚未绑定时隐藏默认新会话用量，完成权威恢复后才展示。

最近上下文占用与会话累计实报是不同事实。SDK 的 `SessionTokenUsage` 保留明细与缺报调用数，
累计权威由 AppServer 持久账本拥有，本地具名事实、SQLite 缓存及投影保留全部明细与未知值，
不累加事件。只有完整核心计数参与无窗口时的 used 展示，部分实报仍保留于事实中。
退出快照由根会话 owner 在清空投影前冻结，经既有 conversation 边界交给
`finalize_application`，在统一收尾成功并释放终端后消费一次；命令不各自打印摘要，也不为
展示保留已删除会话的存储。快照区分可恢复、归档、删除完成及未决删除，后者保留原请求身份；
只有远端终态或删除回执能确认相应观察对象停止，本地结束不隐含远端停止。新身份清除旧候选，
恢复其他会话的删除不改写当前快照。统一输出独立判定可靠用量、可恢复身份、归档和未决删除，
恢复命令分行展示；尚未确认远端停止时标注当前已知用量与远端状态。具体语义见
[上下文用量客户端契约](docs/context-usage-protocol.md#退出摘要契约)。
恢复 adapter 将明确的访问拒绝作为具名失败事实交给 Root，由 Root 撤销对应身份的恢复指引；
网络故障和过期查看令牌不推断权限撤销，旧身份的失败不得影响当前身份。
...
- TUI 通过 Harness owner 的类型化快照与控制请求读取和管理外接 MCP；单服务目标、全量目标及取消导航彼此独立。菜单只产生单服务请求，直接命令显式选择全量；前端不读取 SDK 工具组或自行解析配置推断连接状态。
- 配置启用、连接状态和工具数量是不同事实。已观察到断线时撤下该连接的工具；清理未确认时保留资源所有权，不建立重叠连接。
- Harness 通过 MCP owner 的使用范围注入工具来源，根 Turn、子代理、Subscription、Review 与 Hook 共同持有冻结目录。runtime 原子记录逐服务引用并检查交互关闭；有引用时 stop/restart 返回 busy，补启动只进入后续目录。
- 全量 start/force 保留既有连接；本批 required 判定完成后才发布新增目录，失败只回收本批。最终关闭及工作区移交禁止新引用和迟到发布，等待消费者释放；失败资源继续由 Harness owner 持有并重试。
- 可选服务的前台等待预算由既有 runtime 与连接组归约，预算到期后启动任务继续归原 group 所有；重复预热借用同一启动，不重置期限。连接并发上限跨批次共享，关闭收束包括排队与预检任务。后续成功只加入新的工具使用范围。
- `infrastructure.mcp.catalog_cache` 是单 runtime 的内存派生缓存，按工作区、原始服务键、有效配置和 OAuth 凭据代际隔离，受容量、字节数与 TTL 限制。缓存不持有连接或后台任务，退休时清空；配置失效、认证变化和连接失败禁止继续复用。
- 缓存预览不改变连接就绪事实；其使用范围绑定启动 attempt 与完整目录内容版本，并持有同一关闭门禁。缓存移除免审批注解、采用 prompt 且不提供记忆选项，现有权限策略及已获授权仍由审批核心裁决；调用须等待原连接，并验证实时目录及凭据身份，变化时拒绝发送，不迁移或重放调用。SDK 请求排队期间同样检查缓存调用身份；请求异常且发送终态未知时，该约束保持到连接收束。实时目录继续由现有 OAuth 请求边界处理凭据刷新。
- Session grant 只在相同 Session、Environment、server、connector 和 tool 范围内复用。
- 交互审批只拥有待决请求和选择；决定提交后即锁定，终态由结构化事实投影。
- 外接 MCP elicitation 是绑定本地调用的输入交互。`agent.domain.mcp_elicitation` 定义不可变请求和响应，SDK schema 校验及消息分发属于 `infrastructure.mcp`；TUI 只编辑当前请求，既有 `ApprovalCoordinator` 统一持有排队、容量、取消及展示批次。该输入不经过审批核心、grant 或事实存储，不生成 AppServer 审批字段。
- 组合边界仅向具备表单和 URL 展示能力的 runtime 注入交互端。连接内工具调用串行绑定 session、turn、call 和 agent 身份，服务端请求具有独立本地输入 ID；SDK 接收循环继续处理取消和其他响应。等待期限包括排队时间，调用结束或连接关闭收束所属交互；调用异常后关闭连接，禁止迟到答案或自动重放。
- URL 交互由用户明确同意后交给既有浏览器 adapter；表单只接收有界普通字段。交互正文不进入审批历史或客户端协议诊断，SDK 双向消息日志由连接资源栈过滤。
...
运行时的 `infrastructure.mcp.oauth_runtime` 为每个适用的 Streamable HTTP 连接创建
HTTPX 认证适配器，组合根注入与 CLI 相同命名空间的凭据存储。每个请求重读当前版本；
显式 Bearer/Header、stdio 和 SSE 不查 OAuth。空记录允许匿名请求，不要求系统凭据库可用；
已有凭据不可读时显式失败。运行时不发起交互登录，不重放 401/403 或跟随认证重定向。
过期刷新在单目标锁内重读、持久标记消费、交换和提交；标记期间没有可重用的旧令牌。
网络结果不确定、崩溃或保存失败留下 `refresh_uncertain`，明确拒绝继续刷新直到重新登录。
已发送的刷新在调用方取消后仍完成有界交换和保存；锁等待可取消。响应只能失效其使用的
凭据版本，不能覆盖并发登录或刷新。`agent.domain.mcp_authorization.McpAuthorizationStatus`
统一配置类型、本地凭据状态、版本和最后请求观察；`McpServiceSnapshot.authorization`
承载该无机密事实。凭据替换或退出后清除旧认证结论，迟到响应不得标记较新凭据。
显式状态查询由既有 runtime 读取本地凭据并按配置身份合并，不额外探测网络。
CLI 与 TUI 共享展示和恢复提示，TUI 不读取凭据存储；本地存在凭据不代表远端认证已通过。
连接 owner 撤下失效目录并释放 SDK 资源，已结束的 owner 会唤醒等待中的工具调用。
认证不会生成工具审批 grant，Turn、子代理、Hook 和 Subscription 继续借用同一目录。
...
凭据查询只返回 `McpOAuthCredentialView`，展示使用无机密的认证投影；持有快照不表示
远端认证刚刚通过。令牌、client secret、

...
## ⭐️ GPU 推理集群（A10G Inference Fabric）

Mind 的云端增强能力由 **GPU 推理集群**承载：以 **A10G** 为主力计算单元，构建面向“低延迟在线推理 + 批处理回归”的统一推理底座。  
...
1. **BGE-m3 语义召回**：召回候选工具/知识片段/计划模板  
2. **Cross Encoder 精排**：收敛到最优候选（工具/片段/计划）  
3. **视觉模型（灰度 CNN / 彩色 CNN / YOLO11s）**：对关键帧做状态确认与目标定位  
...
### 5) A10G 选型：工程稳定与吞吐密度的黄金平衡

A10G 定位于“稳定在线推理”的甜点区间：

...
<a id="modes"></a>
## ⭐️ 运行模式
**Mind** 提供三种互斥运行模式：`--chat`、`--fast`、`--plan`。  
...
- **互斥参数（Mutually Exclusive）**：一条命令里只能选 **一个**；用于确定“主运行协议/主入口”。  
  典型：`--chat | --fast | --plan`，以及 `--hello | --upgrade` 这类“单一动作入口”。  
...
- 先选一个主入口：`--hello`、`--upgrade`、`--chat`、`--fast`、`--plan`
...
- 需要长样例、长文本字段或完整星图时，直接看独立文档

如果你想先建立结构感，可以先把星图拆成两部分：
...
[loop_prefix]
  ...
  [round_prefix]
    ...
    [item_prefix]
      ...
      [prefix 或 global_prefix]
      任务正文
      [suffix 或 global_suffix]
...
- `loop_* / round_* / item_*` 是真正的外层结构
- `global_prefix / prefix` 是任务正文前置的默认值与覆盖值
- `global_suffix / suffix` 是任务正文后置的默认值与覆盖值
- `global_rule / rule` 是任务规则文本的默认值与覆盖值

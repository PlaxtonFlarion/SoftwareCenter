...
| `plan` | 先出计划，再顺序执行步骤 | 独立计划执行面；排除部分 security/runtime/nexus 工具 | 巡检、固定流程、批处理、需要步骤可读和更强可复盘性时 | 开放式多轮探索、边聊边改策略 |

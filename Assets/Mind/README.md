...
- 设备 / UI 执行：从 [快速开始](#quick-start)、[运行模式](#modes) 和 [设备与 UI 实战](docs/playbook.device.md) 入手
...
- 订阅任务下发 / 长链路监听：看 [订阅模式](docs/agent-mode.md)
...
- 已经知道要找什么：直接走 [专题目录](docs/README.md)
- 准备看协议、模板和安全链路：从 [接口实战](#playbook-api)、[模板能力](docs/playbook.template.md)、[安全工具](docs/playbook.security.md) 继续
- 准备看设备、多媒体和稳定性链路：从 [设备与 UI 实战](docs/playbook.device.md)、[Monkey 扰动](docs/playbook.monkey.md)、[多媒体链路](#playbook-media) 继续
...
- **交互式模式**：进入循环交互，适合探索与调试
- **蓝图协议**：用 `--code` 组织批跑、前后置和回归
...
- 需要订阅监听：再看 `--agent` 和 [订阅模式](docs/agent-mode.md)
- 想留痕归档：再看 `--gravity`
- 想调试链路：再看 `--reflection`
- 想批跑和回归：最后看 `--code`
...
## ⭐️ 性能实战
完整性能案例和星图样例已拆到独立正文：[性能实战](docs/playbook.performance.md)。
...
<a id="playbook-api"></a>
## ⭐️ 接口实战
完整协议约定、helper 迁移和样例说明已拆到独立正文：[接口实战](docs/playbook.api.md)。
...
需要字段级骨架、`env / items / concurrency / fail_fast` 的真实写法时，直接看 [星图协议](docs/cli-code.md) 和 [接口实战](docs/playbook.api.md)。
...
## ⭐️ 多媒体链路
完整媒体命令、组合链路和星图任务已拆到独立正文：[多媒体链路](docs/playbook.media.md)。

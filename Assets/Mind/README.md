...
- [驻留与订阅模式](docs/agent-mode.md)
- [接口实战](#playbook-api)
- [模板能力](docs/playbook.template.md)
- [安全工具](docs/playbook.security.md)
- [设备域实战](docs/playbook.device.md)
- [Monkey 扰动](docs/playbook.monkey.md)
...
- 设备 / UI 执行：从 [快速开始](#quick-start)、[运行模式](#modes) 和 [设备域实战](docs/playbook.device.md) 入手
- 性能与稳定性：看 [性能工具接口层](#performance-tooling) 和 [性能实战](#playbook-performance)
- 接口协议验证：看 [接口实战](#playbook-api)
- 模板 helper / 签名前置：看 [模板能力](docs/playbook.template.md)
- 安全签名 / JWT / AES / RSA：看 [安全工具](docs/playbook.security.md)
- Monkey 稳定性扰动：看 [Monkey 扰动](docs/playbook.monkey.md)
- 音视频与证据链：看 [多媒体链路](#playbook-media)
- 批跑 / 编排 / 回归：看 [命令行参数](#cli-arguments) 中的 `--code`
- 驻留任务下发 / 长链路监听：看 [驻留与订阅模式](docs/agent-mode.md)
...
- 已经知道要找什么：直接走 [Docs 索引](docs/README.md)
- 准备看协议、模板和安全链路：从 [接口实战](#playbook-api)、[模板能力](docs/playbook.template.md)、[安全工具](docs/playbook.security.md) 继续
- 准备看设备、多媒体和稳定性链路：从 [设备域实战](docs/playbook.device.md)、[Monkey 扰动](docs/playbook.monkey.md)、[多媒体链路](#playbook-media) 继续
...
- 选一个适合当前任务的入口：`chat / fast / plan`，或者驻留订阅用的 `agent`
...
- **驻留订阅模式**：`mind --agent`，本地注册驻留会话并等待服务端下发任务，适合远端调度与长链路监听
...
除此之外，还有一条独立的驻留订阅入口：`--agent`。  
它们的差异不在“模型强弱”，而在“任务形态”和“执行边界”。

补充判断：

- `chat / fast / plan`：本地主动发起一次任务
- `agent`：本地先驻留，再等待服务端下发任务
- `agent` 不属于 REPL 内的第四个状态；协议细节见 [驻留与订阅模式](docs/agent-mode.md)
...
- `free_rule` 指执行期的自由规则判断能力，只在 `plan` 执行面使用
- `--code` 里的 `global_rule / rule` 属于星图规则层，不等同于 `free_rule`

常见误判：

- `fast` 不是“轻量版 chat”，而是专门为短链路收窄过的执行面
- `plan` 不是“所有能力都更强”，它强调的是步骤稳定和顺序执行
- 接口能力主要落在 `bench.nexus`，不是单独的 `api` 工具域
...
- `mind --agent`：适合驻留监听、远端任务下发和恢复重连链路
...
| 驻留订阅监听 | `mind --agent` | 等待服务端下发任务、维持长链路 |
...
- 需要驻留监听：再看 `--agent` 和 [docs/agent-mode.md](docs/agent-mode.md)
- 想留痕归档：再看 `--gravity`
- 想调试链路：再看 `--reflection`
- 想批跑和回归：最后看 `--code`
...
- 每个槽位包含：`api / type / model / apikey / base_url`
- 可查看本地运行日志
- 可查看 MCP 服务状态
- 命令：`mind --hello`
...
- 真正的字段、层级和规则写法，直接看 [docs/cli-code.md](docs/cli-code.md)
...
高级批跑说明已拆到独立文档：[docs/cli-code-advanced.md](docs/cli-code-advanced.md)。
...
- 这里的 `global_rule / rule` 属于星图规则层，不等同于 `plan` 执行面的 `free_rule`
...
## ⭐️ 性能实战
完整性能案例和星图样例已拆到独立文档：[docs/playbook.performance.md](docs/playbook.performance.md)。
...
<a id="playbook-api"></a>
## ⭐️ 接口实战
完整协议约定、helper 迁移和样例说明已拆到独立文档：[docs/playbook.api.md](docs/playbook.api.md)。
...
需要字段级骨架、`env / items / concurrency / fail_fast` 的真实写法时，直接看 [docs/cli-code.md](docs/cli-code.md) 和 [docs/playbook.api.md](docs/playbook.api.md)。
...
## ⭐️ 多媒体链路
完整媒体命令、组合链路和星图任务已拆到独立文档：[docs/playbook.media.md](docs/playbook.media.md)。
...
3. 更新文档：新增/变更能力需同步 README；工具说明与维护约定看 `docs/maintainer-guide.md`

...
| `xtra` | 外接工具与编码协作 | 数据库、浏览器、外部 MCP 服务、Helix 通用工具、原生 coding 和 Codex 链路 | 设备/UI 执行、重型性能链路 |
...
| 外接工具与编码协作 | `mind --xtra "..."` | 数据库、浏览器、外部 MCP 服务、原生 coding 或 Codex 协作 |
...
用于外接 MCP 与编码协作链路，暴露外接 MCP 工具、Helix 通用工具、原生 coding 工具和 Codex 链路。

- 适用于：数据库、浏览器、外部服务、第三方 MCP 工具和编码任务协作
- 不等同于 `--fast`：请求会以 `mode=xtra` 进入独立后端链路
- 适合把 DBHub、浏览器自动化、外部知识库、原生 coding 和 Codex 等能力接进本轮任务
- 命令：`mind --xtra "..."`

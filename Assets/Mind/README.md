...
- **[Monkey 扰动实战](docs/monkey-playbook.md)**
...
- **按场景跳转**
- **设备 / UI 执行**：从 [快速开始](#quick-start)、[Top10 核心能力](#top10) 和 [设备域实战](docs/device-playbook.md) 入手
...
- **Monkey 稳定性扰动**：看 [Monkey 扰动实战](docs/monkey-playbook.md)
...
- 需要系统看 `device` 域的能力分层和稳定执行建议时，直接看 [设备域实战](docs/device-playbook.md)
- 需要系统看 `device.monkey` 的参数、执行流程和返回结构时，直接看 [Monkey 扰动实战](docs/monkey-playbook.md)
...
### 能力边界对齐
三种模式的核心差异，不是“模型强弱”，而是“任务形态”和“执行边界”：

| 模式     | 任务形态         | 能力边界                                                     | 更适合                        | 不建议主打                    |
|--------|------------------|----------------------------------------------------------|----------------------------|--------------------------|
| `chat` | 边探索边推进       | 最宽；适合自然语言驱动的混合任务                                  | 探索、问答、临时任务、混合型接口/设备操作     | 追求最稳定步骤形态时               |
| `fast` | 短链路快速完成     | 更收敛；优先服务接口、媒体、文件这类短路径                           | 接口请求、事件流采样、媒体处理、短路径任务     | 设备/UI 执行、重型性能链路、需要全域工具时 |
| `plan` | 先定步骤再稳定执行 | 最适合固定流程、巡检、回归和需要规则判断的任务                         | 巡检、固定流程、批处理、需要步骤可读和更强可复盘性时 | 开放式多轮探索、边聊边改策略           |
...
- `mind --fast "..."`：适合接口、媒体、文件类短链路；不要拿它做设备/UI 执行
...
## ⭐️ 性能实战教学
完整性能案例和星图样例已拆到独立文档：[docs/performance-playbook.md](docs/performance-playbook.md)。
...
需要字段级骨架、`env / items / concurrency / fail_fast` 的真实写法时，直接看 [docs/cli-code.md](docs/cli-code.md) 和 [docs/api-playbook.md](docs/api-playbook.md)。
...
## ⭐️ 多媒体链路实战教学
完整媒体命令、组合链路和星图任务已拆到独立文档：[docs/media-playbook.md](docs/media-playbook.md)。

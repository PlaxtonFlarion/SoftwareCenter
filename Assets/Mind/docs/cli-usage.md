...
全量 MCP 启动默认对每个可选服务等待 1 秒，预算从本批启动时刻共同计算。
预算用完后服务显示 `starting`，继续在后台建立连接；成功的工具只加入后续工具使用范围，
不会改变正在执行的 Turn。`required = true` 的服务优先启动并等待完整结果，失败时本批
不发布新目录。单服务显式启动仍等待该服务的完成结果。
...
`optional_startup_wait_sec = 0` 表示完整等待；它只控制前台等待，不延长连接本身的启动期限。
重复启动会合并正在进行的连接，停止、重启或退出会回收后台连接及排队任务。

同一运行实例可复用最近的工具目录，最多保留 32 个服务、总计 16 MiB，30 分钟过期；
不写入磁盘，工作区退休或进程退出时清空。配置、过滤规则、审批设置或认证身份不同不能
复用旧目录。缓存目录不代表连接就绪，`/mcp status` 仍展示实际连接事实。
缓存中的工具不携带免审批依据，需要展示审批时只提供允许本次或拒绝；现有权限模式及
已经授予的会话权限继续生效。调用会等待原连接完成，并核对实时工具定义和凭据代际，
在 SDK 实际发送前再次检查身份。若发生变化则拒绝本次调用，后续工具使用范围采用新目录。
缓存失效不会修改已冻结 Turn 的工具描述或自动重放调用。
...
支持 elicitation 的外接 MCP 服务可以在工具执行中请求补充信息。交互式 TUI 会显示来源服务、
代理和请求说明：选择字段编辑，核对最终值后选择 `Submit`，或选择 `Decline` / `Cancel`。
`Esc` 在字段编辑中返回预览，在预览页取消当前请求；并行请求依次展示，取消一张表单不会取消后面的表单。
这类输入不会创建工具授权或记忆选择，也不会替代原有工具审批。

表单支持字符串、整数、有限数值、布尔值、字符串单选和多选，以及默认值、必填、范围、长度、
条目数和 `email` / `uri` / `date` / `date-time` 格式校验。字符串编辑支持多行及空字符串，
最多同时显示四行并跟随光标滚动。布尔和单选编辑默认选中当前值；多选切换后保留当前位置，
选择 `Done` 完成编辑。方向键移动、回车确认，也可以使用选项前的数字直接选择。
调整终端尺寸会重排当前菜单并保留尚未提交的输入。
每个请求最多 64 KiB、32 个字段，每字段最多 4096 个字符，枚举最多 64 项；嵌套对象、任意数组、
正则 `pattern` 和其他不支持的 schema 约束会明确报错。密码、令牌、API key、卡号等敏感信息应通过
服务的网页流程处理；客户端拒绝已识别的敏感字段名或标题、密码格式和 `writeOnly` 字段。

URL 请求先展示目标域名和完整地址，只有选择 `Open URL` 才调用系统浏览器；打开失败返回 `cancel`。
地址只接受 HTTPS 或 loopback HTTP，不接受内嵌用户名、密码、控制字符或脚本地址。
浏览器打开成功只代表接收了网页交互请求，网页上的最终结果由服务处理；此流程与 `mcp login` 的连接认证独立。

只有实际支持两种交互的前端才声明相应 MCP 能力。非交互入口、没有绑定调用身份的请求以及冻结策略为
`approval_policy=never` 的调用返回 `decline`。等待输入最多 120 秒（包含排队），现有工具超时可能更早结束。
超时、调用方取消、服务端取消或连接关闭会清理对应表面；迟到回答不会导航或交付。
交互连接中的工具调用按顺序执行，不阻塞其他连接；异常调用会关闭所属连接，可在调用范围释放后通过
`/mcp restart <name>` 恢复。客户端不会因此重放工具，取消输入也不撤销工具此前已完成的效果。

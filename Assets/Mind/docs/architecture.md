...
- **约束优先**：`chat / fast` 都是流式对话驱动的 `tool_call -> tool_result` 闭环；`chat` 先产出计划，再按步骤顺序执行
...
补充：
- Mind 除了 `chat / fast / xtra` 四种本地主动执行通道，还提供 `agent` 订阅入口
- `agent` 负责向服务端注册会话，并把远端下发任务映射回本地 `chat / fast / xtra` 执行面
...
- **CHAT**：先生成结构化步骤链，再顺序执行，适合需要可读步骤和更稳路径的任务

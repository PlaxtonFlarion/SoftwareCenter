...
README 只保留入口层信息；如果你需要系统理解 `bench.nexus` 的模板 helper、能力边界和签名前置写法，直接看这里。
...
- 在请求地址里直接拼模板变量，例如把租户标识写进 `/api/{{ tenant }}/profile`
- 在请求头里直接生成动态值，例如把 `X-Request-Id` 写成 `{{ uuid4() }}`，把 `X-Timestamp` 写成 `{{ now_s() }}`

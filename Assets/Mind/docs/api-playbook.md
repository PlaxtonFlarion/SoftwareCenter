...
- `asserts`：按 `{path, op, value}` 结构对原始结果做通过条件判断

最小样例只需要记住这三件事：
- `request` 放协议原生字段
- `extract` 只提真正需要回看的字段
- `asserts` 只写真正决定通过与否的条件
...
例如你要检查一个 health 接口，
request 里放请求方法和地址，
extract 里只提取状态码和业务 ok 字段，
asserts 里只写“状态码等于 200”“业务 ok 等于 true”这类通过条件。
```
...
如果你要把接口工具真正写进 `--code`，推荐先看 [星图协议深入说明](cli-code.md) 里的“复杂接口蓝本示例”。

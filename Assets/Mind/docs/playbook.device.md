...
主 README 只负责入口和模式；`device` 域的边界、分层和稳定执行建议继续看这里。
重点是先判断当前动作该不该落在 `device` 域，再拆清应用、页面、系统和取证这些层级。

## 先判断是不是 device 域问题

- 你要真的对设备做动作：比如启动应用、切页面、点击输入、滚动、截图、导日志，看这里
- 你要做端侧收敛、等待、诊断修复、多设备广播执行，看这里
- 你主要在处理接口协议、媒体转码或安全签名，不要先从 device 文档开始
- 你只是想确认主入口、模式和最小命令，先回 README，不必先读完整设备文档
...
设备域负责真的操作设备；媒体域负责截图、录屏、音视频和证据加工；性能与协议能力落在 bench；`--code` 负责把动作组织成可批跑、可回归的星图。

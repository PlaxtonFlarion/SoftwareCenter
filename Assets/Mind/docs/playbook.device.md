...
- 如果你要把设备动作真正写进 `mind batch`，结构层先配合星图协议页，执行语义再配合星图深入说明
...
- 工具参数使用 `serial` 选择目标设备
- 只有一台设备在线时，`serial` 可以省略
- 多台设备在线且未提供 `serial` 时，工具会要求显式指定目标设备

这意味着：

- 你不写 `serial`，只适合单设备在线场景
- 多设备批量执行应由调用方发起多次工具调用，每次传不同 `serial`
- 设备动作不会再隐式广播到所有在线设备
...
- 刷新在线设备列表
- 在执行前收敛可用设备
- 为后续设备动作提供可用的 `serial` 列表

适合：

- 执行前做设备就绪检查
- 多设备批跑前先确认在线设备集合，并在后续调用中显式传 `serial`
...
- 多设备场景要显式传 `serial`，不要依赖隐式默认设备
- 文本输入依赖输入焦点和输入法环境
- 兜底式输入方案对中文、emoji、特殊字符兼容性有限
- `xpath` 不适合当成长期稳定定位策略
- 坐标点击容易随分辨率、布局和状态变化漂移
- `reboot`、文件删除、清应用数据这类动作有明显副作用，别在探索场景乱用

## 和其它域的关系

设备与 UI 执行负责真的操作设备；多媒体处理负责截图、录屏、音视频和证据加工；协议与校验、性能采样负责接口与报告链路；`mind batch` 负责把动作组织成可批跑、可回归的星图。

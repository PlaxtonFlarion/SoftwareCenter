...
## E2E 耗时、ASR 首字上屏、VAD 尾字上屏、流式 tokens/s (E2E, ASR, VAD, Tokens/s)
``````
```cfg
attempts: 3
stop_on_fail: false
...
## Android 内存基线 (Android Memory Baseline)
``````
```cfg
repeat: 10
...
## Android 内存泄漏 (Android Memory Leak)
``````
```cfg
loop_suffix: |
  生成内存测试报告
...
## Android 流畅度 (Android Smoothness)
``````
```cfg
loop_suffix: |
  生成流畅度测试报告

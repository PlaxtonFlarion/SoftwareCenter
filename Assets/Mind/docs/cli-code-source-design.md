...
这份文档面向维护者和实现者，说明当前 `mind flow` 的统一 source 入口，以及它如何兼容命令行、本地交互、服务调用和订阅链路。
...
- 你只想知道 `mind flow` 现在怎么写：先看 [星图协议](cli-code.md)
...
当前实现里，`mind flow` 的主语义已经是“来源列表”，支持：
...
把 `mind flow` 的输入统一抽象成“来源列表”。所有入口先把调用方提供的内容解析成统一来源，再解析正文，再进入批跑执行面。

...
这份文档面向维护者和实现者，说明当前 `mind batch` 的统一 source 入口，以及它如何兼容命令行、本地交互、服务调用和订阅链路。

重点不是改星图语法，而是说明“怎么定位一份星图内容”已经从单一路径模型收敛成统一来源模型。

一句话目标：

- `code` 已经按“来源模式”理解；本机路径只是其中一种兼容 source。

## 先判断这页的范围

- 你只想知道 `mind batch` 现在怎么写：先看 [星图协议](cli-code.md)
- 你只想知道执行顺序、覆盖优先级和批跑控制语义：先看 [星图深入说明](cli-code-advanced.md)
- 你要确认星图输入来源的当前实现边界：看这里
- 你要让命令行、交互、服务调用和订阅链路共享同一套输入语义：看这里

## 当前实现口径

当前实现里，`mind batch` 的主语义已经是“来源列表”，支持：

- 本地文件路径
- `-` 标准输入
- `inline:<内容>` 内联星图文本
- `http://` 或 `https://` 远端文本来源
- 一次传入多个 source，按传入顺序顺序执行

这解决了早期“只能传本机文件路径”的限制。剩余设计问题主要集中在服务端协议治理、远端来源安全策略和平台制品抽象。
...
- 本机文件、stdin、inline 和 URL 都只是内容定位方式
- 任务内容本身仍然是解析后的星图文本
- 外部协议应优先传 inline、URL 或平台制品，而不是端侧路径
...
- 统一来源抽象已经存在，但平台级白名单、审计和制品治理仍应继续增强

## 设计目标

### 核心目标

- `mind batch` 不以“本机路径”作为唯一输入语义
- 所有入口统一收敛到同一种来源抽象
- 星图解析继续只消费文本，不感知来源差异
...
把 `mind batch` 的输入统一抽象成“来源列表”。所有入口先把调用方提供的内容解析成统一来源，再解析正文，再进入批跑执行面。
...
- 适合统一权限、统一追踪和统一版本管理
- 是后续平台化扩展，不属于当前本地 CLI 的直接 source 形态
...
- 批跑入口已消费统一来源。
- 命令行已支持本地文件、stdin、inline 和 URL。
- 批跑执行器按统一 `CodeSourceResolved` 消费文本，不直接把“路径”当全部输入语义。

### 后续

继续增强远端来源、平台制品、缓存、鉴权和来源审计。

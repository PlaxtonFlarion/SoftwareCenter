...
- `/delete`：永久删除当前会话及其子会话并退出前台；菜单默认取消，确认绑定当前身份
...
语言选择器、`/effort` 的菜单标题和选项支持中英文；通用菜单的确认、返回、关闭页脚、
搜索默认提示、无匹配结果及当前/默认标记使用所选语言，按键名称跟随实际快捷键配置。
新打开或更新的菜单使用当前语言；已经打开的模态菜单继续原有交互，切换不会重复提交选择。
默认搜索支持显示的中文标签和原有搜索标识。

其他功能菜单的业务标题、说明和操作反馈尚未完成双语迁移，自定义提示保留原文。
命令名及参数仍使用原名称，例如 `/model`；模型回复、会话正文和第三方内容保留原有语言。
...
- `/delete`：永久删除当前会话及其子会话。菜单默认选择取消，展示不可撤销警告与子会话范围；确认后再次核对打开菜单时的身份，远端和本地清理完整完成才退出。
- `/delete recover <请求ID>`：按失败提示中的原请求 ID 查询并继续清理，可在重启后使用。恢复不会重新提交删除；结果未知时目标不能继续发送或恢复。只有服务端明确拒绝最初提交，才允许重新操作该会话。远端已删除但本地清理失败时，也使用此命令；已删除身份不会出现在退出恢复提示中。恢复其他会话完成后保留当前会话。
...
- 完成记录原位接替活动标题，并撤下说明行；立即接续的 Thinking 使用交接腾出的空间，首帧不挤动完成标题。PostCompact Hook 尚未结束时也会先显示已经确认的完成事实，输入屏障仍由前台操作持有。只有行首使用粗点 `•`，耗时与附加状态使用小点 `·`。
- 自动压缩完成后直接接续 Thinking 或正文，不插入横线；完成记录同时收束此前工具工作段，本轮结束时也不再为该段补画横线。此后新执行的工具按正常工作段展示。
- 后台终端状态按真实数量显示，窄屏按整段省略 `/stop`、`/ps` 和终端信息；窗口恢复后重新显示。压缩失败显示服务端错误分类对应的原因，自动模式等待所属 Turn 的权威中断终态后显示 interrupted。
- 手动压缩开始后按 Esc 只停止等待，并提示服务端继续执行。断线、超时或网关错误保持远端结果未知；恢复会话时，客户端通过已记录的 Item 身份读取远端 completed/failed，恢复权威耗时且不重复执行 Hook。尚未观察到 Item 身份或历史已裁剪时保持未知，不重提压缩请求。
...
空配置只显示关闭提示；配置损坏时仍列出已持有的连接，保留最近有效配置中的启用状态，允许查看状态和停止。
...
OAuth 认证失败时，状态中的 `Authorization` 显示安全错误码。按

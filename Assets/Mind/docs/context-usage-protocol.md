...
摘要具有独立调用事实，但尚未接入持久累计。缺失缓存或推理明细保持未知，不能由客户端补成零；
...
此样例包含用于验证累计值不影响百分比的合成账本值；当前 AppServer 尚未接入实际调用账本，
...
未知、明细不全、缺报调用和只有总数的情况。服务端真实生产值仍为 null；启用非空累计前需要
完整调用记录、数据库基线与原子提交，且客户端已支持完整结构。契约推送不代表服务已部署。
...
Session，发布累计变化时保留主上下文 last/window/source/model/route；这条账本更新路径尚待
接入。detached Review 只更新自身 Session，不使根会话最近占用从未知变为已知。
